...
public class WinstoneBindingEnumeration implements NamingEnumeration
{
  ...
  /**
   * Constructor - sets up the enumeration ready for retrieving bindings instead of
   * NameClassPairs.
   * 
   * @param bindings The source binding set
   */
  public WinstoneBindingEnumeration(Hashtable bindings, Hashtable environment, 
    Context context, WinstoneResourceBundle resources)
  {
  ...
  public Object next() throws NamingException
  {
    if (this.nameEnumeration == null)
      throw new NamingException(this.resources.getString("WinstoneBindingEnumeration.AlreadyClosed"));
      
    String name = (String) this.nameEnumeration.nextElement();
    Object value = this.bindings.get(name);
    try 
    {
      value = NamingManager.getObjectInstance(value, new CompositeName().add(name), 
        this.context, this.contextEnvironment);
    } 
    catch (Throwable err) 
    {
      NamingException errNaming = new NamingException(this.resources.getString("WinstoneBindingEnumeration.FailedToGetInstance"));
  ...
  {
    try {return hasMore();}
    catch (NamingException err) {return false;}
  }

  public Object nextElement()
  {
    try {return next();}
    catch (NamingException err) {return null;}
  }
}
