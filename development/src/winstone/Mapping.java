...
public class Mapping implements java.util.Comparator
{
  ...
  private String linkName; // used to map filters to a specific servlet by name
  private String mappedTo;
  private int patternType;
  private boolean isPatternFirst; // ie is this a blah* pattern, not *blah (extensions only)
  ...
  /**
   * Constructor - this parses the url pattern into pieces we can use to match against
   * incoming URLs.
   */
  ...
  public int getPatternType() 	{return this.patternType;}
  public String getUrlPattern() {return this.urlPattern;}
  public String getMappedTo()		{return this.mappedTo;}
  ...
  /**
   * Used to compare two url patterns. Always sorts so that lowest pattern type then
   * longest path come first.
   */
  public int compare(Object objOne, Object objTwo)
  {
    ...
    int order = -1 * new Integer(one.getPatternType()).compareTo(new Integer(two.getPatternType()));
    return (order != 0 ? order : -1 * one.getUrlPattern().compareTo(two.getUrlPattern()));
  }
  
  public String toString() 
  {
    return this.linkName != null ? "Link:" + this.linkName
        : "URLPattern:type=" + this.patternType + ",pattern=" + this.urlPattern;
  }
}
